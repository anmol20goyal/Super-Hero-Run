...
public class BotManager2 : MonoBehaviour
{
	#region GameObjects
	...
	[NonSerialized] public bool bot2Lost;
	[NonSerialized] public bool bot2Won;

	#endregion

	#region InstanceBM

	private static BotManager2 instance;

	public static BotManager2 InstanceBM2
	{
		get
		{
			if (instance == null)
			{
				instance = FindObjectOfType<BotManager2>();
			}
	...
	private void checkForCurrentBot()
	{
		if (currentBotname == "bot_yeti")
		{
			...
			movingSpeed = (GameHandler.InstanceGH.speed - 0.5f) * 0.75f;
			changeAnimations();
		}
		else if (currentBotname == "bot_ninja")
		{
			...
			movingSpeed = GameHandler.InstanceGH.speed - 0.5f;
			changeAnimations();
		}
		else if (currentBotname == "bot_superhero")
		{
			// flying player
			botsPlayers.layer = 11;
			currentBotname = "bot_superhero";
			movingSpeed = GameHandler.InstanceGH.speed - 0.5f;
			changeAnimations();
			botsPlayers.transform.localPosition = new Vector3(botsPlayers.transform.localPosition.x, height, botsPlayers.transform.localPosition.z);
			if (allowMove)
			{
				botsPlayers.transform.localRotation = Quaternion.Euler(rotateAngle, 0, 0);
				UpDownMoveFly();
			}
		}
	...
	private void UpDownMoveFly()
	{
		if (!gameEnded)
		{
			...
			Vector3 position1 = botsPlayers.transform.localPosition + diffPos;
			Vector3 position2 = botsPlayers.transform.localPosition - diffPos;

			botsPlayers.transform.localPosition = Vector3.Lerp(position1, position2, (Mathf.Sin(Time.time * 3) + 1) / 2);
		}
	}
	...
	private void rayCasting()
	{
		...
		if (Physics.Raycast(pos, Vector3.forward, out hit, 8, myLayerMask) && hit.collider.gameObject.layer == 10 &&
		    forOnce && currentBotname != "bot_superhero")
		{
			// Debug.Log("flying");
			changeToFlyingBot();
			forOnce = false;
		}
		else if (Physics.Raycast(pos2, Vector3.forward, out hit1, 7, myLayerMask) && currentBotname != "bot_ninja" && for2)
		{
			// Debug.Log("ninja");
			if (currentBotname == "bot_superhero")
			{
				StartCoroutine(rotateLanding());
			}
	...
	private void changeToFlyingBot()
	{
		botsPlayers.transform.localScale = Vector3.one * 2;
		currentBotname = "bot_superhero";
	...
	{
		StartCoroutine(changeHeight(botsPlayers.transform.localPosition.y, 8f, 0.3f));
		iTween.RotateTo(botsPlayers,iTween.Hash("x", rotateAngle, "time", 0.3f, "islocal", true, "easetype", "linear"));
		yield return new WaitForSeconds(0.3f);
	}

	private IEnumerator rotateLanding()
	{
		StartCoroutine(changeHeight(height, 0.5f, 0.3f));
		iTween.RotateTo(botsPlayers, iTween.Hash("x", 0, "time", 0.3f, "islocal", true, "easetype", "linear"));
		yield return new WaitForSeconds(0.3f);
	}
	...
	{
		float elapsed = 0.0f;
		var pos = botsPlayers.transform.localPosition;
		while (elapsed < duration)
		{
			height = Mathf.Lerp(h_start, h_end, elapsed / duration);
			botsPlayers.transform.localPosition = new Vector3(pos.x, height, pos.z);
		...
		if (obst != null)
		{
			foreach (GameObject ob in obst)
			{
				if (ob.transform.localPosition.z + 5 < botsPlayers.transform.localPosition.z &&
				    ob.transform.localPosition.x <= -10f && ob.transform.localPosition.x > -15f)
				{
					Destroy(ob);
				}
			}
